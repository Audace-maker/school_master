    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="card-header d-flex justify-content-between align-items-center" style="display: flex; justify-content: space-between; align-items: center;">
            <h3 class="card-title" style="margin: 0;">Liste des classes</h3>
                                    ...
                                    <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" name="section" id="section" >
                                                    ...
                                                    <option value="0">Pas de section</option>
                                                    <?php foreach ($section as $p) { ?>
                                                        <option value="<?= $p['ID_SECTION'] ?>"><?= $p['DESCRIPTION'] ?></option>
                                                    <?php } ?>
                                                </select>
                                                <span class="help-block" style="color:red"></span>
                                            </div>
...
<script type="text/javascript">
    ...
    function liste() {
        ...
        var url = "<?= base_url() ?>administrations/Classe/liste/";
...
$('.modal-title').text('Nouvelle classe');
}
</script>
...
<script type="text/javascript">
    ...
    if (save_method == "add") {
      url = "<?php echo base_url('index.php/administrations/Classe/add') ?>";
      successMessage = "Enregistrement effectué avec succès!";
    } else {
      url = "<?php echo base_url('index.php/administrations/Classe/update') ?>";
...
<script>
    ...
    $.ajax({
      url: "<?php echo site_url('administrations/Classe/getOne') ?>/" + id,
      type: "GET",
      dataType: "JSON",
      success: function (data) {
        $('[name="ID_CLASSE"]').val(data.ID_CLASSE);
        $('[name="section"]').val(data.ID_SECTION);
        $('[name="classe"]').val(data.NOM);


        $('#user_modal').modal('show');
        $('.modal-title').text('Modification de la classe');
...
<script>
    function change_status_classe(id, stat) {
        ...
        if (stat == 0) {
            message = 'Voulez-vous activer cette classe ?';
        } else {
            message = 'Voulez-vous arrêter cette classe ?';
        }
        idField.value = id; 
        ...
        $('#confChangeStatus').off('click').on('click', function() {
            ...
            $.ajax({
                url: "<?php echo base_url('index.php/administrations/Classe/change_status_classe')?>/" + id + '/' + stat,
                type: "POST",
                dataType: "JSON",
                data: {
