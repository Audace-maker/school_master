    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="card-header d-flex justify-content-between align-items-center" style="display: flex; justify-content: space-between; align-items: center;">
            <h3 class="card-title" style="margin: 0;">Liste des sections</h3>
...
<script type="text/javascript">
    ...
    function liste() {
        ...
        var url = "<?= base_url() ?>administrations/Section/liste/";
...
$('.modal-title').text('Nouvelle section');
}
</script>
...
<script type="text/javascript">
    ...
    if (save_method == "add") {
      url = "<?php echo base_url('index.php/administrations/Section/add') ?>";
      successMessage = "Enregistrement effectué avec succès!";
    } else {
      url = "<?php echo base_url('index.php/administrations/Section/update') ?>";
...
<script>
    ...
    $.ajax({
      url: "<?php echo site_url('administrations/Section/getOne') ?>/" + id,
      type: "GET",
      dataType: "JSON",
      success: function (data) {
        ...
        $('#user_modal').modal('show');
        $('.modal-title').text('Modification de la section');
...
<script>
    function change_status_section(id, stat) {
        ...
        if (stat == 0) {
            message = 'Voulez-vous activer cette section ?';
        } else {
            message = 'Voulez-vous arrêter cette section ?';
        }
        idField.value = id; 
        ...
        $('#confChangeStatus').off('click').on('click', function() {
            ...
            $.ajax({
                url: "<?php echo base_url('index.php/administrations/Section/change_status_section')?>/" + id + '/' + stat,
                type: "POST",
                dataType: "JSON",
                data: {
