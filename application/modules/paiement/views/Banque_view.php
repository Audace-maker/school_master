    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="card-header d-flex justify-content-between align-items-center" style="display: flex; justify-content: space-between; align-items: center;">
            <h3 class="card-title" style="margin: 0;">Liste des Banques</h3>
            <button type="button" class="btn btn-primary" style="margin-left: auto;" onclick="new_banque()">Ajouter</button>
        </div>
              <!-- /.card-header -->
              <div class="card-body">
...
<script type="text/javascript">
    ...
    function liste() {
        ...
        var url = "<?= base_url() ?>paiement/Banque/liste/";
...
$('.modal-title').text('Nouvelle Banque');
}
</script>
...
<script type="text/javascript">
    ...
    if (save_method == "add") {
      url = "<?php echo base_url('index.php/paiement/Banque/add') ?>";
      successMessage = "Enregistrement effectué avec succès!";
    } else {
      url = "<?php echo base_url('index.php/paiement/Banque/update') ?>";
...
<script>
    ...
    $.ajax({
      url: "<?php echo site_url('paiement/Banque/getOne') ?>/" + id,
      type: "GET",
      dataType: "JSON",
      success: function (data) {
        $('[name="ID_BANQUE"]').val(data.ID_BANQUE);
        $('[name="nom"]').val(data.NOM);


        $('#user_modal').modal('show');
        $('.modal-title').text('Modification de la banque');
...
<script>
    function change_status_banque(id, stat) {
        ...
        if (stat == 0) {
            message = 'Voulez-vous activer cette Banque?';
        } else {
            message = 'Voulez-vous arrêter cette Banque ?';
        }
        idField.value = id; 
        ...
        $('#confChangeStatus').off('click').on('click', function() {
            ...
            $.ajax({
                url: "<?php echo base_url('index.php/paiement/Banque/change_status_banque')?>/" + id + '/' + stat,
                type: "POST",
                dataType: "JSON",
                data: {
                ...
                <div class="row">
                    <div class="col-md-12">
                        <form id="users_form" method="POST">
                            <div class="form-body">
                                <input type="hidden" name="ID_BANQUE" id="ID_BANQUE">
